...
$all_11_c = $wpdb->get_var("SELECT COUNT(id) FROM $wpdb->zibpay_order WHERE `order_type` = 11 AND `status` = 1");
$all_12_c = $wpdb->get_var("SELECT COUNT(id) FROM $wpdb->zibpay_order WHERE `order_type` = 12 AND `status` = 1");
     ...
     <div class="order-header">
        ...
        <div class="flex">
            <ul class="subsubsub">
                ...
                <li class=""><a class="" href="<?php echo $order_url . '&amp;order_type=11'; ?>">打赏作者</a>(<?php echo $all_11_c ?>)</li> |
                <li class=""><a class="" href="<?php echo $order_url . '&amp;order_type=12'; ?>">购买广告</a>(<?php echo $all_12_c ?>)</li> |
...
if ($list) {
    ...
    foreach ($list as $value) {
        ...
        if ($value->order_type == 11 && $order_type == '付费内容') {
            $order_type = '打赏作者';
        }
        if ($value->order_type == 12 && $order_type == '付费内容') {
            $order_type = '购买广告';
        }

        $order_type_link = '<a style="color: ' . ['#ff4747', '#ee5307', '#1e8608', '#1a8a65', '#0c9cc8', '#086ae8', '#3353fd', '#4641e8', '#853bf2', '#e94df7', '#ca2b7d', '#d7354c', '#ff4747', '#8e24ac', '#ff8c00', '#6c757d'][(int) $value->order_type] . ';" href="' . $order_url . '&order_type=' . $value->order_type . '">' . $order_type . '</a>';
        $user_a          = '未登录用户';
        if ($value->user_id) {
